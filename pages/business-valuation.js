...
export default function BusinessValuation() {
  ...
  const generatePDF = () => {
    ...
    if (formData.includeRealEstate === 'yes') {
      doc.text(`+ Real Estate: $${formData.realEstateValue}`, 20, y);
      y += lineHeight;
    ...
    y += lineHeight;
    doc.setFont('helvetica', 'normal');
    const financingText = 'Offering seller financing under your own terms can increase your total payout. For example, financing $250K over 4 years at 6% interest could add tens of thousands in interest income to your sale price.';
    doc.text(financingText, 20, y, { maxWidth: 170 });
    y += (Math.ceil(doc.getTextDimensions(financingText).h / lineHeight) + 2) * lineHeight;
