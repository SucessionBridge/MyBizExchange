...
async function insertMessage(row) {
  // Ensure buyer_name not null if buyer_email is present
  if (!row.buyer_name && row.buyer_email) row.buyer_name = row.buyer_email;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      let   buyer_name   = coerceNull(fields.buyer_name);
      ...
      const from_seller  = ['true','1','yes'].includes(String(first(fields.from_seller) || '').toLowerCase());
      ...
      if (!buyer_name && buyer_email) buyer_name = buyer_email;
      ...
      const row = {
        ...
        attachments,
        from_seller: Boolean(from_seller),
      };
    ...
    let   buyer_name   = coerceNull(body.buyer_name);
    ...
    const from_seller  = Boolean(body.from_seller);
